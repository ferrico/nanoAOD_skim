...
/// KinFitter header
#include "../interface/KinZfitter.h"
...
void KinZfitter::Setup(std::vector< std::vector< float > > selectedLeptons, std::map<unsigned int, TLorentzVector> selectedFsrPhotons, int year){
...
//             if(p=="AlphaL_1")   { AlphaL_1_ = val;}
//             if(p=="ExpL_1")   { ExpL_1_ = val;}
//             if(p=="AlphaR_1")   { AlphaR_1_ = val;}
//             if(p=="ExpR_1")   { ExpR_1_ = val;}


            if(p=="f2")   { f2_ = val;}
            if(p=="meanGauss2")   { meanGauss2_ = val;}
            if(p=="sigmaGauss2")   { sigmaGauss2_ = val;}
//             if(p=="AlphaL_2")   { AlphaL_2_ = val;}
//             if(p=="ExpL_2")   { ExpL_2_ = val;}
//             if(p=="AlphaR_2")   { AlphaR_2_ = val;}
//             if(p=="ExpR_2")   { ExpR_2_ = val;}

            if(p=="f3")   { f3_ = val;}
            if(p=="meanGauss3")   { meanGauss3_ = val;}
            if(p=="sigmaGauss3")   { sigmaGauss3_ = val;}
//             if(p=="AlphaL_3")   { AlphaL_3_ = val;}
//             if(p=="ExpL_3")   { ExpL_3_ = val;}
//             if(p=="AlphaR_3")   { AlphaR_3_ = val;}
//             if(p=="ExpR_3")   { ExpR_3_ = val;}
...
void KinZfitter::Setup(std::vector<TLorentzVector> VtxLep, std::vector<double> lep_pTError, std::vector< reco::Candidate* > selectedLeptons, std::map<unsigned int, TLorentzVector> selectedFsrPhotons, int year){
    ...
    while (!input.eof() && std::getline(input,line))
    {
        ...
        if(iss >> p >> val) {
            ...
            if(p=="mean" )  { meanVal_ = val;}//  cout<<"VXBS mean is  "<<meanVal_<<endl;}
            if(p=="sigma" )  { sigmaVal_ = val;}//  cout<<"VXBS sigma is  "<<sigmaVal_<<endl;}
//             if(p=="f1")   { f1Val_ = val;}//  cout<<"f1 is  "<<f1Val_<<endl;}
...
void KinZfitter::initZs(std::vector< std::vector<float> > selectedLeptons, std::map<unsigned int, TLorentzVector> selectedFsrPhotons, int year){
	...
	std::vector<float> c = selectedLeptons[il];              
        pTerr = helperFunc_->pterr(c,  isData_, year);
        p4.SetPtEtaPhiM(c[0], c[1], c[2], c[3]);
        int pdgId = c[4];
...
void KinZfitter::MakeModel(/*RooWorkspace &w,*/ KinZfitter::FitInput &input, KinZfitter::FitOutput &output) {
    ...
    RooFormulaVar* mZ_2;
     ...
     if (input.nFsr == 1) {
        ...
        mZ_2 = new RooFormulaVar("mZ_2", "TMath::Sqrt(2*@0+2*@1+2*@2+2*@3+2*@4+2*@5+@6*@6+@7*@7)", RooArgList(p1D2,p1Dph1,p2Dph1,p1Dph2,p2Dph2,ph1Dph2, m1, m2));
        }
     else {
         mZ_2 = new RooFormulaVar("mZ_2", "TMath::Sqrt(2*@0+@1*@1+@2*@2)", RooArgList(p1D2, m1, m2));
     }

     double mZ_val = mZ_2->getVal();

// Parametro proxy per il constraint
      RooRealVar mZ("mZ", "constraint on mZ", mZ_val);
...
//      RooBreitWigner BW("BW","BW", *mZ, meanCB, sigmaCB);

     RooRealVar f1("f1","",f1_);
     RooRealVar meanGauss1("meanGauss1","",meanGauss1_);
     RooRealVar sigmaGauss1("sigmaGauss1","",sigmaGauss1_);
//      RooRealVar AlphaL_1("AlphaL_1","",AlphaL_1_);
//      RooRealVar ExpL_1("ExpL_1","",ExpL_1_);
//      RooRealVar AlphaR_1("AlphaR_1","",AlphaR_1_);
//      RooRealVar ExpR_1("ExpR_1","",ExpR_1_);
//      RooMyPDF_DSCB DSCB_1("DSCB_1", "DSCB_1", *mZ, meanGauss1, sigmaGauss1, AlphaL_1, ExpL_1, AlphaR_1, ExpR_1);


     RooRealVar f2("f2","",f2_);
     RooRealVar meanGauss2("meanGauss2","",meanGauss2_);
     RooRealVar sigmaGauss2("sigmaGauss2","",sigmaGauss2_);
//      RooRealVar AlphaL_2("AlphaL_2","",AlphaL_2_);
//      RooRealVar ExpL_2("ExpL_2","",ExpL_2_);
//      RooRealVar AlphaR_2("AlphaR_2","",AlphaR_2_);
//      RooRealVar ExpR_2("ExpR_2","",ExpR_2_);
//      RooMyPDF_DSCB DSCB_2("DSCB_2", "DSCB_2", *mZ, meanGauss2, sigmaGauss2, AlphaL_2, ExpL_2, AlphaR_2, ExpR_2);

     RooRealVar f3("f3","",f3_);
     RooRealVar meanGauss3("meanGauss3","",meanGauss3_);
     RooRealVar sigmaGauss3("sigmaGauss3","",sigmaGauss3_);
//      RooRealVar AlphaL_3("AlphaL_3","",AlphaL_3_);
//      RooRealVar ExpL_3("ExpL_3","",ExpL_3_);
//      RooRealVar AlphaR_3("AlphaR_3","",AlphaR_3_);
//      RooRealVar ExpR_3("ExpR_3","",ExpR_3_);
//      RooMyPDF_DSCB DSCB_3("DSCB_3", "DSCB_3", *mZ, meanGauss3, sigmaGauss3, AlphaL_3, ExpL_3, AlphaR_3, ExpR_3);
...
//      RooAddPdf* BWplusDSCB = new RooAddPdf("BWplusDSCB", "", BW, DSCB_1, f1);
//      RooAddPdf* BWplusDSCBplusDSCB = new RooAddPdf("BWplusDSBWplusDSCB", "", *BWplusDSCB, DSCB_2, f2);
//      RooAddPdf* BWplusDSCBplusDSCBplusDSCB = new RooAddPdf("CBplusDSCBplusDSCBplusDSCB", "", *BWplusDSCBplusDSCB, DSCB_3, f3);
...
//std::cout<<meanCB.getVal()<<"\t"<<sigmaCB.getVal()<<std::endl;
...
//     delete BWplusDSCB;
//     delete *DSCB_2;
//     delete BWplusDSCBplusDSCB;
//     delete *DSCB_3;
//     delete BWplusDSCBplusDSCBplusDSCB;
    delete model;
...
// void KinZfitter::RooMyPDF_DSCB(const char *name, const char *title,
//                         RooAbsReal& _x,
//                         RooAbsReal& _mean,
//                         RooAbsReal& _sigma,
//                         RooAbsReal& _alphaL,
...
//  }
// 
// void KinZfitter::RooMyPDF_DSCB(const RooMyPDF_DSCB& other, const char* name) :
...
// 
// 
//  double KinZfitter::evaluate() const
//  {
